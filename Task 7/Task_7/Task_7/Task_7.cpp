...
public:
	...
	vector<vector<int> > Connections(int n, vector<vector<int> >& a)
	{
		...
		for (int i = 1; i <= n; i++) {
			...
			if (!is_scc[i]) {
				...
				for (int j = i + 1; j <= n; j++) {
					...
					if (!is_scc[j] && isPath(i, j, ad_list)
						&& isPath(j, i, ad_list)) {
						is_scc[j] = 1;
						scc.push_back(j);
					}
				...
				strong_connections.push_back(scc);
			}
		}
...
int main()
{
	...
	vector<vector<int> > ans = obj.Connections(V, edges);
